...
megasas_set_high_iops_queue_affinity_and_hint(struct megasas_instance *instance)
{
	...
	if (instance->perf_mode == MR_BALANCED_PERF_MODE) {
		mask = cpumask_of_node(dev_to_node(&instance->pdev->dev));//对于9540出现过call trace
...
static int megasas_init_fw(struct megasas_instance *instance)
{
	...
	if (msix_enable && !msix_disable) {
		...
		instance->msix_vectors = min(num_msix_req,
				instance->msix_vectors);
		megasas_alloc_irq_vectors(instance);//对于9540 调用该函数出现过call trace
		if (!instance->msix_vectors)
			instance->msix_load_balance = false;
	}
	/*
	...
	//调这：.init_adapter = megasas_init_adapter_fusion,最终到megasas_init_adapter_fusion
...
static int megasas_alloc_ctrl_mem(struct megasas_instance *instance)
{
	...
	case MFI_SERIES:
		if (megasas_alloc_mfi_ctrl_mem(instance))
			return -ENOMEM;
		break;
	case AERO_SERIES:  // 9540-8i类型是这个（5） 
	case VENTURA_SERIES:// 9460-8i类型是这个（4） 
